...
var getFriendsSpecific = async (player) => {
    var result = await db.promise().query("SELECT friends.id AS frid,friends.accepted AS frac,us1.id AS us1id,us1.username AS us1us,us1.avatar AS us1av,us2.id AS us2id,us2.username AS us2us, us2.avatar AS us2av FROM friends JOIN users AS us1 ON friends.offeror=us1.id JOIN users AS us2 ON friends.offeree=us2.id WHERE offeror=? OR offeree=? GROUP BY friends.id",[player,player])
    var arr = Array();
    for(var i = 0; i < result[0].length; i++) {
        let obj = new Object();
        obj.id = result[0][i].frid;
        obj.offeror = {"id":result[0][i].us1id,"username":result[0][i].us1us,"avatar":"http://"+ip.address()+":8000"+result[0][i].us1av};
        obj.offeree = {"id":result[0][i].us2id,"username":result[0][i].us2us,"avatar":"http://"+ip.address()+":8000"+result[0][i].us2av};
